...
class CreditNoteTest < Test::Unit::TestCase
  ...
  def test_credit_note_sub_total_calculation
    ...
    expected_sub_total = credit_note.line_items.inject(BigDecimal('0')) { | sum, line_item | line_item.line_amount }
  ...
  def test_credit_note_sub_total_calculation
    ...
    # Make sure that everything adds up to begin with.
    expected_total_tax = credit_note.line_items.inject(BigDecimal('0')) { | sum, line_item | line_item.tax_amount }
